...
def clean_dataset(df):
    ...
    if 'lbxglu' in df_clean.columns:
        ...
        if len(outliers) > 0:
            print(f"  ⚠ Found {len(outliers)} glucose outliers (< 50 or > 400)")
...
def validate_dataset(df):
    ...
    print(f"\n✓ Missing values: {total_missing} ({missing_pct:.2f}% of total)")
...
def generate_summary_report(df, output_path):
    ...
    report.append("  - lbxin: Insulin (μU/mL)")
    report.append("")
